/**
 * Deterministic CPU noise used for world generation (island shape, rocks, placement).
 * Simplex noise after Stefan Gustavson, with a seeded permutation table.
 */
...
export class Noise {
  ...
  /** Ridged multifractal in [0,1]: sharp crests, used for cliffs and erosion ridges. */
  ridged2(x: number, y: number, octaves = 5): number {
    let sum = 0;
    let amp = 0.5;
    let prev = 1;
    for (let i = 0; i < octaves; i++) {
